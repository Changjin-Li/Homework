...
class Config:
    def __init__(self, lr, weight_decay, epochs, mode = "train"):
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.fc2 = nn.Sequential(
            nn.Linear(self.config.hidden_size * self.config.direction, self.config.num_classes),
        )
        self.fc3 = nn.Sequential(
            nn.Linear(len(self.config.filter_size) * self.config.num_filters + self.config.hidden_size * self.config.direction, self.config.num_classes),
        )
        self.relu = nn.ReLU()
...
def main():
    ...
    if net == "cnn":
        model.train_cnn()
        train(Config(1e-4, 1e-6, 30), model, train_data, dev_data)
    elif net == "rnn":
        model.train_rnn()
        train(Config(5e-4, 1e-3, 30), model, train_data, dev_data)
