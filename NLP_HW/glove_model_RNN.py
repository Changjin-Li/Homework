...
class Model(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.config.hidden_size * self.config.direction + len(self.config.filter_size) * self.config.num_filters, self.config.num_classes),
        )
        self.relu = nn.ReLU()
    ...
    def load_glove(self):
        ...
        csv_file = open('model/glove.300d/word_vector_test.csv', "w", newline='')
        name = ['vector']
        try:
    ...
    def word2vec_init(self):
        # self.load_glove()
        init_weight = pd.read_csv('dataset/word_vector.csv')['vector'].tolist()
    ...
    def forward(self, x):
        ...
        feature, _ = self.gru(vec)                  # BxLx(H*2)
        rnn_feature = feature[:, -1, :]             # Bx(H*2)
        ...
        for conv in self.conv:
            ...
            out.append(h)
        cnn_feature = torch.cat(out, 1)
        feature = torch.cat([rnn_feature, cnn_feature], 1)
