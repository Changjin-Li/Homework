...
def main():
    ...
    canvas_h, canvas_w = height * 2, width * 5
    canvas = np.zeros((canvas_h, canvas_w, 3), dtype=np.uint8)
    num_eigenfaces = min(10, eigenvectors.shape[1])
    for i in range(num_eigenfaces):
        eigenvector = eigenvectors[:, i].reshape((height, -1))
        eigen_norm = norm_0_255(eigenvector)
        eigen_color = cv2.applyColorMap(eigen_norm, cv2.COLORMAP_JET)
        if len(sys.argv) == 2:
            cv2.imshow(f"eigenface_{i}", eigen_color)
        else:
    ...
    results = []
    max_components = min(eigenvectors.shape[1], 300)
    start = min(eigenvectors.shape[1], 10)
    for num_components in range(start, max_components, 15):
        evs = eigenvectors[:, :num_components]
        projection = subspaceProject(evs, mean, first_img_flat)
        ...
        if len(sys.argv) == 2:
            cv2.imshow(f"eigenface_reconstruction_{num_components}", recon_norm)
        else:
            results.append(recon_norm)
            cv2.imwrite(os.path.join(output_folder, f"eigenface_reconstruction_{num_components}.png"), recon_norm)

    canvas_h, canvas_w = height * 4, width * 5
    canvas = np.zeros((canvas_h, canvas_w), dtype=np.uint8)
    for i in range(20):
        canvas[(i // 5) * height: (i // 5 + 1) * height, (i % 5) * width: (i % 5 + 1) * width] = results[i]
    cv2.imwrite(os.path.join(output_folder, "eigenface_reconstructions.png"), canvas)
